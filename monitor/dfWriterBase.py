...
class dfWriterBase:
  def writeHistoricalDfTo(self, base_path='', data_frame=pd.DataFrame()):
  ...
  def writeDailyDfTo(self, base_path='', data_frame=pd.DataFrame()):
    lines_num = data_frame.shape[0]
    ...
    for i in range(1, lines_num):
      ...
      if last_filename != curr_filename or i >= lines_num - 1:
        ...
        if not path.exists(filepath):
          ...
          data_frame[date_last_index: date_end_index].to_csv(path_or_buf=file_ptr, date_format='%Y-%m-%d-%H-%M-%S')
          file_ptr.close()
        else:
          self.appendDfToFullPathDaily(filepath, data_frame[date_last_index: date_end_index])

        last_index = i
        last_filename = curr_filename

  def appendDfToFullPathDaily(self, full_path='', data_frame=pd.DataFrame()):
    ...
    f = open("test1.csv", "w")
    result.to_csv(path_or_buf=f, date_format='%Y-%m-%d-%H-%M-%S')
    f.close()
